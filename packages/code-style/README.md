...
> **0.0.6**: пакет в активной разработке. API может меняться без предупреждения.
...
```bash
npm install eslint@^10.4.0 prettier@3.9.5 typescript@6.0.3 --save-dev
```

Опционально:

```bash
npm install stylelint@17.14.0 --save-dev     # CSS/SCSS линтинг
npm install oxlint@1.73.0 --save-dev         # гибридный режим OxLint
npm install @biomejs/biome@2.5.4 --save-dev  # гибридный режим Biome
```
...
```bash
npm install stylelint@17.14.0 --save-dev
```
...
Если проект гоняет и dependency-cruiser, и eslint-FSD, передай `createFSDConfig({ dependencyCruiser: true })`. Из eslint уберутся два правила, которые dependency-cruiser заменяет полностью: `import-x/no-restricted-paths` (направление слоёв) и `@37bytes/no-slice-self-import`. `import-x/no-internal-modules` остаётся намеренно: dependency-cruiser покрывает только его FSD-половину (public API слайсов), а запрет deep-import в резолвящиеся внешние пакеты (`пакет/lib/внутренний/файл`) не покрывает никто в графе. Также остаются порядок импортов, запрет legacy-папок, server-only/client-only первой строкой. Побочный эффект: нарушения public API FSD будут под двойным контролем (eslint в редакторе + dependency-cruiser в CI), но это редундантный сигнал на реальном нарушении, не шум. Передавай флаг, только если реально запускаешь dependency-cruiser в CI, иначе направление слоёв не проверяет никто.

### Установка

```bash
npm install dependency-cruiser@17.4.3 typescript@6.0.3 --save-dev
```
