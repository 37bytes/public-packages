 ...
 */
import { executePackageBinarySync } from '#tests/package-binary';
...
// package binary resolution throws here instead of silently catch-continuing inside the per-rule loop
// (which would otherwise produce an empty, misleading domain-rules file).
const biomeVersionOutput = executePackageBinarySync('@biomejs/biome', 'biome', ['--version'], {
    encoding: 'utf8'
});
...
for (const [categoryName, categoryDefinition] of Object.entries(schema.$defs)) {
    ...
    for (const ruleName of Object.keys(categoryDefinition.properties)) {
        ...
        try {
            ...
            explanation = executePackageBinarySync('@biomejs/biome', 'biome', ['explain', ruleName], {
                encoding: 'utf8',
