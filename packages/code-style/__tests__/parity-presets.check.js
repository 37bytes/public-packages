import { nextjs, nodejsRuntime, nodejsTool, spa, testingConfig } from '#eslint';
import { executePackageBinary } from '#tests/package-binary';
import { knownGaps } from '#tests/parity/known-gaps';
import { reverseToEslint } from '#tests/parity/rule-equivalence';
...
const withRunnerName = async (name, runnerPromise) => {
    try {
        ...
        error.message = `${name} runner failed: ${error.message}`;
        throw error;
    }
};
